...
## C# (.NET 8)
- Projects: Library (`csharp/McapCs`), Tests (`csharp/McapCs.Test`), App (`csharp/McapFileVerificationApp`). Solution: `McapCs.sln` at repo root.
- Target framework: `net8.0`.
- Tests: xUnit (`xunit`, `xunit.runner.visualstudio`) with `Microsoft.NET.Test.Sdk`; coverage via `coverlet.collector`.
- Naming: .NET 標準 (PascalCase: 型/メンバー、camelCase: ローカル変数)。
- Nullability: `<Nullable>enable</Nullable>`; implicit `using` 有効。
- 推奨ツール: `dotnet format`（任意）、`dotnet test --collect:\"XPlat Code Coverage\"`。
...
## Go / Rust
- Follow language norms (gofmt/golangci, rustfmt/clippy if applicable).
