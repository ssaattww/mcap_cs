...
`ZStdWriter` は、`IChunkWriter` インターフェースを実装し、一時バッファにデータを保持してからZStandard圧縮バッファにフラッシュするインメモリライターです。
...
*   `ZStdWriter(CompressionLevel compressionLevel, uint64_t chunkSize)`: 圧縮レベルとチャンクサイズを受け取るコンストラクタ。
*   `~ZStdWriter()`: Zstandard圧縮コンテキストを解放するデストラクタ。
*   `handleWrite(const std::byte* data, uint64_t size)`: 非圧縮バッファにデータを書き込みます。
*   `end()`: 非圧縮バッファのデータをZstandardアルゴリズムで圧縮し、圧縮バッファに格納します。
*   `size() const`: 非圧縮データのサイズを返します。
*   `compressedSize() const`: 圧縮後のデータのサイズを返します。
*   `empty() const`: 非圧縮バッファが空かどうかを返します。
*   `handleClear()`: 非圧縮および圧縮バッファをクリアし、Zstandard圧縮コンテキストをリセットします。
